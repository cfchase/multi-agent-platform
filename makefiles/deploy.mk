...
generate-admin-secret-dev:
	@if ! kubectl get secret admin-credentials -n deep-research-dev >/dev/null 2>&1; then \
...
get-admin-credentials: ## Show admin credentials and URLs for LangFlow/Langfuse/MLFlow
	...
	@echo "========================================"
	@echo "Email:    $$(kubectl get secret admin-credentials -n deep-research-dev -o jsonpath='{.data.email}' | base64 -d)"
	@echo "Password: $$(kubectl get secret admin-credentials -n deep-research-dev -o jsonpath='{.data.password}' | base64 -d)"
	@echo ""
	@echo "SERVICE URLS"
	@echo "========================================"
	@APPS_DOMAIN=$$(oc get ingresses.config.openshift.io cluster -o jsonpath='{.spec.domain}' 2>/dev/null || echo ""); \
	if [ -n "$$APPS_DOMAIN" ]; then \
		echo "Deep Research: https://deep-research-route-deep-research-dev.$$APPS_DOMAIN"; \
		echo "LangFlow:      https://langflow-deep-research-dev.$$APPS_DOMAIN"; \
		echo "Langfuse:      https://langfuse-deep-research-dev.$$APPS_DOMAIN"; \
		echo "MLFlow:        https://mlflow-deep-research-dev.$$APPS_DOMAIN"; \
	else \
		echo "Run 'kubectl get routes -n deep-research-dev' to see URLs"; \
	fi
	@echo "========================================"
...
deploy-app: ## Deploy Deep Research app only (requires postgres-secret)
